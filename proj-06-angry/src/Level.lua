...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        if types['Player'] then
            self.launchMarker.alien_collided = true
        end
        ...
        if types['Obstacle'] and types['Alien'] then
            ...
            if a:getUserData() == 'Obstacle' then
                local velX, velY = a:getBody():getLinearVelocity()
                local velSum = math.abs(velX) + math.abs(velY)
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        if xPos >= 0 and xPos <= VIRTUAL_WIDTH and (math.abs(xVel) + math.abs(yVel) >= 1.5) then
                aliens_stopped = false
            end
        end

        if aliens_stopped then
