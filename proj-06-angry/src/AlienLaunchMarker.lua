...
function AlienLaunchMarker:init(world)
    ...
    self.alien_split = false
    
    self.alien_collided = false
end
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.alien_split and not self.alien_collided then
            self.alien_split = true

            local mainAlien = self.aliens[1]
            local posX, posY = mainAlien.body:getPosition()
            local velX, velY = mainAlien.body:getLinearVelocity()

            -- spawn top alien
            local t_alien = Alien(self.world, 'round', posX, posY, 'Player')
            table.insert(self.aliens, t_alien)
            t_alien.body:setLinearVelocity(velX, velY - 25)
            t_alien.fixture:setRestitution(0.4)
            t_alien.body:setAngularDamping(1)

            -- spawn bottom alien
            local b_alien = Alien(self.world, 'round', posX, posY, 'Player')
            table.insert(self.aliens, b_alien)
            b_alien.body:setLinearVelocity(velX, velY + 25)
            b_alien.fixture:setRestitution(0.4)
            b_alien.body:setAngularDamping(1)
